...
with open('../bus_project_data/schedule.pkl') as f:
    sched = pickle.load(f)
...
for trip_id in sched.trips.index:
    ...
    for i, row in enumerate(trip_stops.iterrows()):
        ...
        edge_tuple = (last_stop, stop_node, dict(trip_id=trip_id, route_id=route_id, \
                     ...
                     duration=ride_time))
        if service_id == 1:
            G_ride_weekday.add_edges_from([edge_tuple])
        if service_id == 2:
            G_ride_saturday.add_edges_from([edge_tuple])
        if service_id == 3:
            G_ride_sunday.add_edges_from([edge_tuple])
